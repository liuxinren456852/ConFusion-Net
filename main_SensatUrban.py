...
from ConFusionNet import Network
...
class SensatUrban: # SensatUrban properties
    def __init__(self):
        self.name = 'SensatUrban' # dataset name
        root_path = '/home/kukdo/data/Workspace/RandLA-SensatUrban/Dataset'  # dataset path
        ...
        self.names_to_label = { v:k for k, v in self.label_to_names.items()}
        ...
        '''
        # debug mode        
        self.all_files = np.sort(glob.glob(join(self.path, 'test', '*.ply')))
        ...
        self.val_file_name = ['birmingham_block_1',
                              'birmingham_block_5',
                              'cambridge_block_10',
                              'cambridge_block_7'] # validation set
        
        self.test_file_name = ['birmingham_block_1',
                              'birmingham_block_5',
                              'cambridge_block_10',
                              'cambridge_block_7'] # test set            
        '''
        self.test_file_name = ['birmingham_block_2', 'birmingham_block_8',
                               'cambridge_block_15', 'cambridge_block_22',
                               'cambridge_block_16', 'cambridge_block_27'] # test set'''
    ...
    def get_tf_mapping2():
        ...
        def tf_map(batch_xyz, batch_features, batch_labels, batch_pc_idx, batch_cloud_idx):
            raw_features = tf.concat([batch_xyz, batch_features], axis=-1)
            input_points = []
            input_colors = []
            input_neighbors = []
            input_pools = []
            ...
            for i in range(cfg.num_layers):
                batch_p_c = tf.concat([batch_xyz, batch_features], axis=-1)
                neighbour_idx = tf.py_func(DP.knn_search, [batch_xyz, batch_xyz, cfg.k_n], tf.int32)
                sub_features = batch_p_c[:, :tf.shape(batch_xyz)[1] // cfg.sub_sampling_ratio[i], :]
                sub_points = sub_features[:, :, :3]
                sub_colors = sub_features[:, :, 3:]
                pool_i = neighbour_idx[:, :tf.shape(batch_xyz)[1] // cfg.sub_sampling_ratio[i], :]
                up_i = tf.py_func(DP.knn_search, [sub_points, batch_xyz, 1], tf.int32)

                input_points.append(batch_xyz)
                input_colors.append(batch_features)
                input_neighbors.append(neighbour_idx)
                input_pools.append(pool_i)
                input_up_samples.append(up_i)

                batch_xyz = sub_points
                batch_features =sub_colors

            input_list = input_points + input_colors + input_neighbors + input_pools + input_up_samples
            input_list += [raw_features, batch_labels, batch_pc_idx, batch_cloud_idx]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='train', help='options: train, test, vis [default: train]')
