...
# import pandas as pd # none accessed?
...
class ConfigSensatUrban:
    ...
    lr_decays = {i: 0.99 for i in range(0, 500)}  # decay rate of learning rate
...
class ConfigToronto3D:
    k_n = 16  # KNN
    num_layers = 5  # Number of layers
    num_points = 131072  # Number of input points
    num_classes = 8  # Number of valid classes
    sub_grid_size = 0.05  # preprocess_parameter
...
class DataProcessing:
    ...
    def get_class_weights(dataset_name):
        ...
        if dataset_name is 'S3DIS':
            ...
            num_per_class = np.array([41697357, 1745448, 6572572, 19136493, 674897, 897825, 4634634, 374721], dtype=np.int32)
        elif dataset_name is 'SensatUrban':
            num_per_class = np.array([29106775, 40111926, 58645441, 1589569, 192203, 2697813, 15696, 
                                      8331315, 1703727, 2268333, 3133575, 11966, 464968], dtype=np.int32)
...
class Plot:
    ...
    def draw_pc_sem_ins(pc_xyz, pc_sem_ins, plot_colors=None):
        ...
        if plot_colors is not None:
            ...
            # ins_colors = Plot.random_colors(len(np.unique(pc_sem_ins)) + 1, seed=2) # random color
            # SensatUrban color scheme
    ...
    def save_ply_o3d(data, save_name):
        ...
        if np.shape(data)[1] == 3:
            ...
            if np.max(data[:, 3:6]) > 20:
                pcd.colors = open3d.utility.Vector3dVector(data[:, 3:6] / 255.)
            else:
                pcd.colors = open3d.utility.Vector3dVector(data[:, 3:6])
            open3d.io.write_point_cloud(save_name, pcd)
        return
